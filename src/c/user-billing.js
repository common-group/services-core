...
const userBilling = {
    controller(args) {
        ...
        const user = args.user,
              ...
              popularBanks = [
                {id: '51', code: '001', name: 'Banco do Brasil S.A.'},
                {id: '131', code: '341', name: 'Itaú Unibanco S.A.'},
                {id: '122', code: '104', name: 'Caixa Econômica Federal'},
                {id: '104', code: '033', name: 'Banco Santander  (Brasil)  S.A.'},
                {id: '127', code: '399', name: 'HSBC Bank Brasil S.A. - Banco Múltiplo'},
                {id: '23', code: '237', name: 'Banco Bradesco S.A.'}
              ];

        userVM.getUserBankAccount(user_id).then(data => bankAccount(_.first(data)));
        userVM.getUserCreditCards(user_id).then(creditCards);
                                      ...
                                      [
                                          m('option[value=\'\']'),
                                            (_.map(ctrl.popularBanks, (bank) => {
                                                return m(`option[value='${bank.id}']`, {selected: bankAccount.bank_id == bank.id},
                                                  `${bank.code} . ${bank.name}`);
                                            })),
                                            (_.find(ctrl.popularBanks, (bank) => { return bank.id === bankAccount.bank_id;}) ? '' :
                                              m(`option[value='${bankAccount.bank_id}']`, {selected: true},
