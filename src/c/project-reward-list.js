...
const projectRewardList = {
  controller () {
    ...
    const submitContribution = () => {
      ...
      const shippingFee = vm.shippingFeeForCurrentReward(fees)

      if (valueFloat < vm.selectedReward().minimum_value + shippingFee) {
        vm.error(`O valor de apoio para essa recompensa deve ser de no mínimo R$${vm.selectedReward().minimum_value} + frete R$${h.formatNumber(shippingFee)}`)
      } else {
        vm.error('')
