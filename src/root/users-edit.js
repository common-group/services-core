...
const usersEdit = {
    controller(args) {
        const userDetails = m.prop({}),
              ...
              displayTabContent = (user) => {
                  const tabs = {
                    '#projects': m(userCreated, {userId}),
                    '#contributions': m(userPrivateContributed, {userId, user}),
                    '#about_me': m(userAboutEdit, {userId, user}),
                    '#settings': m(userSettings, {userId, user}),
                    '#notifications': m(userNotifications, {userId, user}),
                    '#billing': m(userBilling, {userId, user})
                };
    ...
    view(ctrl, args) {
        ...
        return m('div', [
          ...
          (!_.isEmpty(user) ?
            [m('nav.dashboard-nav.u-text-center', {style: {'z-index': '10', 'position': 'relative'}},
              m('.w-container',
                  [
                      ...
                      m(`a.dashboard-nav-link.u-right-big-only[href='/pt/users/${user.id}']`,{config: m.route, onclick: () => {
                              m.route("/users/" + user.id, {user_id: user.id} );}},
