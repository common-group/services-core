...
begin
    data := json_build_object(
        ...
        'customer', json_build_object(
            'name', 'Teste da silva',
            'email', 'test@test.com',
            'document_number', '889.851.228-78',
...
begin
    ...
    return next lives_ok('EXECUTE pay_with_data(''{}'')', 'platform_user can call pay');
...
begin
    ...
    set local "request.header.x-forwarded-for" to '127.0.0.1'; -- ip header should be found
    return next lives_ok('EXECUTE pay_with_data(''{}'')', 'scoped_user can call pay');
    select count(1) from payment_service.catalog_payments
        where user_id = 'd44378a2-3637-447c-9f57-dc20fff574db'
        into _count_expected;
