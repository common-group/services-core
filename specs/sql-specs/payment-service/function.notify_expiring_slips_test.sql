BEGIN;
    ...
    SELECT function_returns('payment_service', 'notify_expiring_slips', '{}'::text[], 'json');
    ...
    as $$
        declare
            ...
            _expiring_payment payment_service.catalog_payments;
        ...
        begin
            ...
            insert into payment_service.catalog_payments
                (status, created_at, gateway, platform_id, user_id, project_id, data, subscription_id, gateway_general_data) values 
                ('pending', (now() - '1 month + 4 days'::interval), 'pagarme', __seed_platform_id(), __seed_first_user_id(), __seed_project_id(), json_build_object('payment_method', 'boleto'), _active_subscription.id, json_build_object('boleto_expiration_date', now() + '1 hour'::interval)::jsonb)
                returning * into _expiring_payment;
            ...
            return next is(((_result ->> 'affected_payments_ids')::json->>0)::uuid, _expiring_payment.id);
        end;
    $$;
