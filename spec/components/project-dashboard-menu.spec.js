describe('ProjectDashboardMenu', () => {
  ...
  describe('view', () => {
    beforeAll(() => {
      generateContextByNewState = (newState = {}) => {
        let body = jasmine.createSpyObj('body', ['className']),
            ...
            component = m.component(ProjectDashboardMenu, {project: projectDetail}),
            ctrl = component.controller({project: projectDetail});

        spyOn(m, 'component').and.callThrough();
        spyOn(ctrl, 'body').and.returnValue(body);

        return {
          output: mq(component.view(ctrl, {project: projectDetail})),
    ...
    it('when project is online', () => {
      let {output, projectDetail} = generateContextByNewState({state: 'online'});
