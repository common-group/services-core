describe('ProjectRewardList', () => {
  ...
  describe('view', () => {
    beforeAll(() => {
      generateContextByNewState = (newState = {}) => {
        ...
        let rewardDetail = RewardDetailsMockery(newState),
            component = m.component(ProjectRewardList, {project: {id: 1231}, rewardDetails: m.prop(rewardDetail)});
    ...
    it('should render card-gone when reward sould out', () => {
      let {output, rewardDetail} = generateContextByNewState({maximum_contributions: 4, paid_count: 4});
    ...
    it('should render card-reward when reward is not sould out', () => {
      let {output, rewardDetail} = generateContextByNewState({maximum_contributions: null});
    ...
    it('should render card-reward stats when reward is limited', () => {
      let {output, rewardDetail} = generateContextByNewState({maximum_contributions: 10, paid_count: 2, waiting_payment_count: 5});
    ...
    it('should render card-reward details', () => {
      let {output, rewardDetail} = generateContextByNewState({minimum_value: 20});
