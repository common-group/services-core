describe('AdminProjectDetailsExplanation', () => {
  let AdminProjectDetailsExplanation = window.c.AdminProjectDetailsExplanation, $output;
  let outputForState = (currentState) => {
    let projectDetail = ProjectDetailsMockery({state: currentState})[0];
    let component = m.component(AdminProjectDetailsExplanation, {resource: projectDetail});
    let view = component.view(component.controller, {resource: projectDetail});
    return mq(view);
  };
