...
CREATE OR REPLACE VIEW "payment_service_api"."subscriptions_per_month" AS 
 SELECT s.project_id,
    ...
    count(DISTINCT s.id) FILTER (WHERE (NOT (EXISTS ( SELECT true AS bool
          ...
          AND ((cp.data ->> 'payment_method'::text) = (past_payments.data ->> 'payment_method'::text)) 
    ...
    COALESCE(sum(((cp.data ->> 'amount'::text))::numeric) FILTER (WHERE (NOT (EXISTS ( SELECT true AS bool
          ...
          WHERE ((past_payments.subscription_id = s.id) AND (past_payments.status = 'paid'::payment_service.payment_status) 
          AND ((cp.data ->> 'payment_method'::text) = (past_payments.data ->> 'payment_method'::text)) 
          AND (past_payments.id <> cp.id) AND (past_payments.created_at < cp.created_at)))))), (0)::numeric) AS new_amount,
  ...
  WHERE ((cp.status = 'paid'::payment_service.payment_status) AND ((s.status <> 'deleted'::payment_service.subscription_status) 
  AND (s.platform_id = core.current_platform_id()) AND (core.is_owner_or_admin(s.user_id) OR core.is_owner_or_admin(p.user_id))))
