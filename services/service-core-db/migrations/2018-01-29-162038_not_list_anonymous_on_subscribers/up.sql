-- Your SQL goes here
grant select on project_service_api.subscribers to scoped_user, anonymous, platform_user;
CREATE OR REPLACE VIEW "project_service_api"."subscribers" AS 
 SELECT u.id,
  ...
  WHERE (u.platform_id = core.current_platform_id()
    AND (
        CASE WHEN core.is_owner_or_admin(p.user_id) OR
            core.is_owner_or_admin(s.user_id) THEN true
        ELSE (s.checkout_data ->> 'anonymous')::boolean is false END
    ))
