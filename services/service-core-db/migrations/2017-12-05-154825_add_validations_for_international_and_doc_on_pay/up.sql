...
AS $function$
        ...
        begin
            ...
            if (_refined->>'is_international') is null OR not (_refined->>'is_international')::boolean then
            ...
            select json_build_object(
                ...
                'customer', json_build_object(
                    ...
                    'phone', json_build_object(
                        'ddi', core_validator.raise_when_empty(regexp_replace(($1->'customer'->'phone'->>'ddi')::text, '[^\d]+', ''), 'phone_ddi'),
                        'ddd', core_validator.raise_when_empty(regexp_replace(($1->'customer'->'phone'->>'ddd')::text, '[^\d]+', ''), 'phone_ddd'),
                        'number', core_validator.raise_when_empty(regexp_replace(($1->'customer'->'phone'->>'number')::text, '[^\d]+', ''), 'phone_number')
                    )
                )
            ) into _result;
