...
CREATE OR REPLACE VIEW "payment_service_api"."subscriptions" AS 
 SELECT s.id,
    ...
    subscriptions_project_data.project_name as project_name
   FROM ((((((((payment_service.subscriptions s
    ...
    LEFT JOIN LATERAL (select project.name as project_name from project_service.projects as project WHERE s.project_id = project.id LIMIT 1 ) subscriptions_project_data ON (true))
