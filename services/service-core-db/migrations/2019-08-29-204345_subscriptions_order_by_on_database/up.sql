CREATE OR REPLACE VIEW "payment_service_api"."subscriptions" AS 
 SELECT s.id,
    ...
    last_paid_payment_date.created_at AS last_paid_payment_created_at,
    ...
    current_paid_subscription.current_reward_external_id,
   ...
   FROM (((((((((payment_service.subscriptions s
     ...
     LEFT JOIN LATERAL ( SELECT cp_version_check.subscription_id,
            ...
            current_reward_data.external_id AS current_reward_external_id
     ...
     LEFT JOIN LATERAL ( SELECT project.name AS project_name
      ...
      LEFT JOIN ( SELECT cp.id,
           ...
           FROM payment_service.catalog_payments cp
          ORDER BY cp.created_at DESC
         LIMIT 1) last_paid_payment_date ON last_paid_payment_date.subscription_id = s.id AND last_paid_payment_date.status = 'paid'::payment_service.payment_status)
  ...
  WHERE ((s.status <> 'deleted'::payment_service.subscription_status) AND (s.platform_id = core.current_platform_id()) AND (core.is_owner_or_admin(s.user_id) OR core.is_owner_or_admin(p.user_id)))
    ORDER BY last_payment.created_at;