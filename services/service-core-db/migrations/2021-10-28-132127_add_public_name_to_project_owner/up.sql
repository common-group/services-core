...
AS $function$
        ...
        begin
            ...
            _result := jsonb_build_object('user', jsonb_build_object(
                ...
                'public_name', _user.data ->> 'public_name',
                'email', _user.email,
            ...
            if _payment.id is not null then
                _result := jsonb_set(_result, '{payment}'::text[], jsonb_build_object(
                    'id', _payment.id,
                    'external_id', _payment.external_id,
                    ...
                    'next_charge_at', (case when _payment.subscription_id is not null then
                        (_payment.created_at + '1 month'::interval)::date
                        else null end),
            ...
            if _subscription.id is not null then
                _result := jsonb_set(_result, '{subscription}'::text[], jsonb_build_object(
                    ...
                    'last_payment_payment_method', (_last_subscription_payment.data ->> 'payment_method')::text,
                    'last_payment_amount', (_last_subscription_payment.data ->> 'amount')::decimal / 100,
                    'payment_method', (_subscription.checkout_data ->> 'payment_method')::text,
                    'amount', (_subscription.checkout_data ->> 'amount')::decimal / 100,
                    ...
                    'next_charge_at', payment_service.next_charge_at(_subscription),
                    'fmt_next_charge_at', payment_service.next_charge_at(_subscription)::timestamp
                ));
            end if;
            ...
            if _project.id is not null then
                _result := jsonb_set(_result, '{project}'::text[], jsonb_build_object(
                    'id', _project.id,
                    'external_id', _project.external_id,
                    'name', _project.name,
                    'mode', _project.mode,
            ...
            if _reward.id is not null then
                _result := jsonb_set(_result, '{reward}'::text[], jsonb_build_object(
                    'id', _reward.id,
                    'external_id', _reward.external_id,
                    'title', _reward.data ->> 'title',
                    'welcome_message_subject', _reward.data ->> 'welcome_message_subject',
                    'welcome_message_body', _reward.data ->> 'welcome_message_body',
            ...
            if _project_owner.id is not null then
                _result := jsonb_set(_result, '{project_owner}'::text[], jsonb_build_object(
                    ...
                    'external_id', _project_owner.external_id,
                    ...
                    'public_name', _project_owner.data ->> 'public_name'
                ));
            end if;
...
CREATE OR REPLACE VIEW "notification_service_api"."user_notifications_with_data" AS
 SELECT n.id,
    n.user_id,
    n.created_at,
    COALESCE(n.label, n.global_template_label) AS label,
    (((n.data -> 'relations'::text) ->> 'project_id'::text))::uuid AS project_id,
     n.data,
     u.external_id as user_external_id,
     u.data ->> 'name' :: text as user_name,
     u.data ->> 'public_name' :: text as user_public_name,
     u.data ->> 'email' :: text as user_email
   FROM notification_service.user_catalog_notifications n
   INNER JOIN community_service.users u on u.id = n.user_id
  WHERE ((n.platform_id = core.current_platform_id()) AND core.is_owner_or_admin(n.user_id));;

grant select on notification_service_api.user_notifications_with_data to admin, platform_user, scoped_user;