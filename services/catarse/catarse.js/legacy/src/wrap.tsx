...
import { useEffect, useState, withHooks, ComponentConstructor } from 'mithril-hooks'
...
function _Wrap(props : WrapProps) {
    ...
    const [myUser, setMyUser] = useState<UserDetails>(null)
        ...
        if (userVM.isLoggedIn) {
            ...
            async function loadMyUserDetails() {
                try {
                    ...
                    setMyUser(await userVM.getMyUser())
                } finally {
    ...
    try {
        ...
        return (
            <div id='app'>
                <If condition={loadingUserDetails || loadingMyUserDetails}>
                    <Loader />
                </If>
                <If condition={!loadingUserDetails && !loadingMyUserDetails}>
                    <HeaderMenu {...parsedProps} user={myUser} />
                    <Component {...parsedProps} />
                </If>
                {notHideFooter && <Footer {...parsedProps} />}
            </div>
        )
    } catch(e) {
