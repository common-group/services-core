class AddTotalSlipFeeInProjectFiscalDataTblRefresh < ActiveRecord::Migration[6.1]
  def up
    execute <<-SQL
      ...
      FROM balance_transfer_requests_projects_view r
        ...
        JOIN LATERAL ( SELECT round(sum(q.value), 2) AS payment_amount,
                ...
                round(sum(q.slip_fee), 2) AS total_slip_fee,
              ...
              FROM ( SELECT date_part('year'::text, zone_timestamp(pa_1.paid_at)) AS year,
                        ...
                        sum(
                          CASE
                            WHEN pa_1.slip_fee > 0 THEN 0
                            ELSE pa_1.gateway_fee
                          END
                        ) AS gateway_fee,
                        sum(aa_1.cost) AS antifraud_cost,
                        sum(pa_1.slip_fee) As slip_fee
                      FROM payments pa_1
