class ChangeServiceFeeAndPaymentMethodFeeToContributionReport < ActiveRecord::Migration[6.1]
  def up
    execute <<-SQL
        DROP VIEW public.contribution_reports_for_project_owners;
        CREATE OR REPLACE VIEW public.contribution_reports_for_project_owners
        AS SELECT b.project_id,
        ...
        CASE
            WHEN pa.slip_fee > 0 THEN numbertocurrency(pa.value * p.service_fee - COALESCE(aa."cost", 0))
            ELSE numbertocurrency(pa.value * p.service_fee - pa.gateway_fee - COALESCE(aa."cost", 0))
        END AS service_fee,
        CASE
            WHEN pa.slip_fee > 0 THEN numbertocurrency(0)
            ELSE numbertocurrency(pa.gateway_fee)
        END AS payment_method_fee,
