...
RSpec.describe Billing::PaymentItems::Chargeback, type: :action do
  describe 'Inputs' do
  ...
  describe '#call' do
    ...
    context 'when payment item cannot transition to charged_back' do
      before { allow(payment_item).to receive(:can_transition_to?).with(:charged_back).and_return(false) }

      it { is_expected.to be_failure }

      it 'returns error message' do
        expect(result.error).to eq 'Payment item cannot transition to charged_back'
      end
    ...
    context 'when payment item can transition to charged_back' do
      it { is_expected.to be_success }

      it 'transitions payment item to charged_back' do
        result

        expect(payment_item.reload).to be_in_state(:charged_back)
      end
    end
