-- Your SQL goes here
DROP view "payment_service_api"."subscriptions_per_month";
CREATE OR REPLACE VIEW "payment_service_api"."subscriptions_per_month" AS 
 SELECT 
    ...
    date_trunc('month', payment_service.paid_transition_at(ROW(cp.id, cp.platform_id, cp.project_id, cp.user_id, cp.subscription_id, cp.reward_id, cp.data, cp.gateway, cp.gateway_cached_data, cp.created_at, cp.updated_at, cp.common_contract_data, cp.gateway_general_data, cp.status, cp.external_id, cp.error_retry_at)))::date as month
