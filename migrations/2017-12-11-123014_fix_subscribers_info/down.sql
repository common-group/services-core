...
AS $function$
        ...
        begin
            ...
            select json_build_object(
                ...
                'new_percent', count(DISTINCT s.user_id) FILTER (WHERE lt.total_contributed_projects = 0)::double precision / greatest((count(DISTINCT s.user_id)::double precision * 100::double precision), 1),
                'returning_percent',count(DISTINCT s.user_id) FILTER (WHERE lt.total_contributed_projects > 0)::double precision / greatest((count(DISTINCT s.user_id)::double precision * 100::double precision), 1)
            )
