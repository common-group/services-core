...
SELECT u.id,
    ...
    json_build_object('public_name', u.data ->> 'public_name'::text, 'name', u.data ->> 'name'::text, 'city', (u.data -> 'address'::text) ->> 'city'::text, 'state', (u.data -> 'address'::text) ->> 'state'::text, 
      'total_contributed_projects', ( SELECT count(DISTINCT cp.project_id) AS count
           FROM payment_service.catalog_payments cp
          WHERE cp.user_id = u.id AND cp.status = 'paid'::payment_service.payment_status AND cp.platform_id = core.current_platform_id()),
      'total_published_projects', ( SELECT count(DISTINCT p_1.id) AS count
