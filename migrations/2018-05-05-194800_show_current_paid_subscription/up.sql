 ...
 SELECT s.id,
   ...
   FROM (((((((payment_service.subscriptions s
     ...
     LEFT JOIN LATERAL ( SELECT
            cp_version_check.subscription_id,
        ...
        FROM payment_service.catalog_payments as cp_version_check
        LEFT JOIN project_service.rewards AS current_reward_data ON current_reward_data.id = cp_version_check.reward_id
        AND cp_version_check.subscription_id = s.id AND cp_version_check.status = 'paid'::payment_service.payment_status
        ORDER BY cp_version_check.created_at DESC LIMIT 1) 
        AS current_paid_subscription ON true)
