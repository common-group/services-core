...
-- missing grants for scoped user use update user
grant usage on community_service.user_versions_id_seq to scoped_user;
grant update, select on community_service.users to scoped_user;

create table project_service.rewards (
    id bigserial not null primary key,
    project_id bigint references project_service.projects(id) not null,
...
create table project_service.reward_versions (
    ...
    reward_id bigint references project_service.rewards(id) not null,
    data jsonb not null default '{}',
    created_at timestamp not null default now(),
    updated_at timestamp not null default now()    
);
...
AS $function$
        ...
        begin
            select json_build_object(
                ...
                'row_order',  ($1->>'row_order')::date,
                'title', core_validator.raise_when_empty(($1->>'title'), 'title')::text,
                'description', core_validator.raise_when_empty(($1->>'description')::text, 'description'),
...
grant select, insert, update on project_service.rewards to scoped_user, platform_user;
grant select, insert, update on project_service.reward_versions to scoped_user, platform_user;
grant usage on sequence project_service.rewards_id_seq to scoped_user, platform_user;
grant usage on sequence project_service.reward_versions_id_seq to scoped_user, platform_user;
...
AS $function$
        ...
        begin
            select json_build_object(
                ...
                'row_order',  coalesce(($1->>'row_order')::date, ($2->>'row_order')::date),
                'title', core_validator.raise_when_empty(
                    coalesce(($1->>'title')::text, ($2->>'title')::text), 'title')::text,
                'description', core_validator.raise_when_empty(
                    coalesce(($1->>'description')::text, ($2->>'description')::text), 'description'),
...
create or replace function project_service_api.reward(data json)
    ...
    $$;
grant execute on function project_service_api.reward(data json) to scoped_user, platform_user;