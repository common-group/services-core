 ...
 SELECT s.id,
   ...
   FROM (((((((payment_service.subscriptions s
     ...
     LEFT JOIN LATERAL ( SELECT cp_version_check.subscription_id,
            ...
            WHERE  
                    cp_version_check.platform_id = s.platform_id
                AND cp_version_check.project_id = s.project_id
                AND cp_version_check.subscription_id = s.id
                AND cp_version_check.user_id = s.user_id
                AND cp_version_check.status = 'paid'::payment_service.payment_status
