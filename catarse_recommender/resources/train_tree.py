...
class TrainTree(Resource):
    ...
    def get(self, num_round=4000, cache=False):
        if not cache:
            ...
            y = ys
            try:
                dump_svmlight_file(features , ys, 'catarse_recommender/common/catarse.txt.all')
                dump_svmlight_file(X_train, y_train, 'catarse_recommender/common/catarse.txt.train')
                dump_svmlight_file(X_test, y_test, 'catarse_recommender/common/catarse.txt.test')
            ...
            dtrain = xgb.DMatrix('catarse_recommender/common/catarse.txt.train', feature_names = self.f_names)
            dtest = xgb.DMatrix('catarse_recommender/common/catarse.txt.test', feature_names = self.f_names)
            data = load_svmlight_file('catarse_recommender/common/catarse.txt.all')
            X = data[0]
            y = data[1]
        ...
        # save model
        bst.save_model('catarse_recommender/common/xgb.model')
