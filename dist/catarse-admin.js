...
adminApp.PaymentBadge = {
    ...
    view: function(ctrl, args) {
        ...
        return m(".w-col.w-col-4", [ m(".fontweight-semibold.fontsize-smaller.lineheight-tighter.u-marginbottom-20", "Recompensa" + (reward.maximum_contributions ? " (limitada)" : "")), m(".fontsize-smallest.lineheight-looser", _.isEmpty(reward) ? "Apoio sem recompensa." : [ "ID: " + reward.id, m("br"), "Valor mínimo: R$" + formatNumber(reward.minimum_value, 2, 3), m("br"), m.trust("Disponíveis: " + available + " / " + (reward.maximum_contributions ? reward.maximum_contributions : "&infin;")), m("br"), "Aguardando confirmação: " + reward.waiting_payment_count, m("br"), "Descrição: " + reward.description ]) ]);
    }
}, adminApp.ToggleDiv = {
    toggler: function() {
