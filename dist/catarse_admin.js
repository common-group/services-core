...
ContributionDetail.get = function(filters, page) {
    ...
    view: function(ctrl) {
        ...
        }) ]), m(".w-section.section", [ m(".w-container", [ m(".w-row", [ m(".w-col.w-col-5"), m(".w-col.w-col-2", [ ctrl.vm.isLoading ? m("button.btn.btn-medium.btn-terciary[href='#']", {
            onclick: ctrl.filter
        }, "Carregar mais") : m("img[alt='Loader'][src='/assets/catarse_bootstrap/loader-eff2ad1eeb09a19c9afb5b143e1dd62b.gif']", {
            style: {
                display: " none"
            }
        }) ]), m(".w-col.w-col-5") ]) ]) ]) ];
    ...
    var contributions = m.prop(""), filters = m.prop(""), isLoading = m.prop(!1), page = m.prop(1), getContributions = function() {
        return adminApp.models.ContributionDetail.get(filters(), page());
    }, filter = function(input) {
        isLoading(!0);
        var input = input || {}, d = m.deferred();
        return filters(input), getContributions().then(function(data) {
            contributions(data), d.resolve(contributions()), isLoading(!1);
        }), d.promise;
    }, nextPage = function() {
        var d = m.deferred();
        return isLoading(!0), page(page() + 1), getContributions().then(function(data) {
            d.resolve(contributions(_.union(contributions(), data))), isLoading(!1);
        }), d.promise;
    };
    return filter(), {
